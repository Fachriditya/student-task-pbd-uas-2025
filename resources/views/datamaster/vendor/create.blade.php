...
<div class="card-table">
    <div class="card-header bg-light">
        <h5 class="mb-0 fw-bold">Form Tambah Vendor</h5>
    </div>
        ...
        <form action="{{ route('vendor.store') }}" method="POST">
            ...
            <div class="form-group mb-3">
                ...
                <select name="badan_hukum" id="badan_hukum" class="form-control" required>
                    ...
                    <option value="C" {{ old('badan_hukum') == 'C' ? 'selected' : '' }}>CV (Persekutuan Komanditer)</option>
                    <option value="U" {{ old('badan_hukum') == 'U' ? 'selected' : '' }}>UD (Usaha Dagang)</option>
                    <option value="F" {{ old('badan_hukum') == 'F' ? 'selected' : '' }}>Firma</option>
                </select>
            </div>
